#ifndef __LCD_H__
#define __LCD_H__

#include "defs.h"


struct __attribute__ ((aligned (16)))  vissprite
{
	byte *buf;
	int x;
	byte pal, pri,pad[6]; 
};

struct  __attribute__ ((aligned (16))) scan 
{
	int bg[64];
	int wnd[64];
	byte buf[256];
	byte pal1[128];
	un16 pal2[64];
	un32 pal4[64];
	byte pri[256];
	struct vissprite vs[16];
	int ns, l, x, y, s, t, u, v, wx, wy, wt, wv;
} ;

struct obj
{
	byte y;
	byte x;
	byte pat;
	byte flags;
};

struct __attribute__ ((aligned (16))) lcd 
{
	byte vbank[2][8192];
	union
	{
		byte mem[256];
		struct obj obj[40];
	} oam;
	byte pal[128];
};

extern struct lcd __attribute__ ((aligned (16))) lcd;
extern struct scan __attribute__ ((aligned (16))) scan;


void lcd_begin();
void lcd_refreshline();
void pal_write(int i, byte b);
void pal_write_dmg(int i, int mapnum, byte d);
void vram_write(int a, byte b);
void pal_dirty();
void vram_dirty();
void lcd_reset();
void bg_scan_color();
void updatepatpix();
void vram_write_range(int a,int cnt,const byte* data);

/* lcdc.c */
void lcdc_trans();
void lcdc_change(byte b);
void stat_write(byte b);
void stat_trigger();
void lcd_init();

#endif
